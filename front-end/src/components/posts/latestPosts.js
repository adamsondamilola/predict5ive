...
const LatestPosts = () =>{
    ...
    const getContents = async () => {
      setLoading(true)      
      fetch(endPoint + 'post/get_posts', options)
...
<p className="text-xl font-semibold pb-5">Sports News 
</p>
...
<div class="grid grid-cols-1 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-4">
        ...
        <article key={x.id} className="max-w-none format lg:format-lg format-red">
                ...
                <div className="flex flex-col justify-start p-4">
                    <Link to={"/post/"+x.id+"/"+x.slug} className="text-1xl font-bold hover:text-gray-700 pb-4">{truncateString(x.post_title, 100)}</Link>
